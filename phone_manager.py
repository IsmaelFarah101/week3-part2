...
class PhoneAssignments():
    ...
    def add_employee(self, employee):
        ...
        for people in self.employees:
            if people.id == employee.id:
                raise PhoneError('There is a employee with the same id')
        self.employees.append(employee)

    def add_phone(self, phone):
        # TODO raise exception if two phones with same ID are added
        for cell in self.phones:
            if cell.id == phone.id:
                raise PhoneError('There is a phone with the same id')
